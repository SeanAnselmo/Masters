...
hist(random_sample$dep_delay, 
     ...
     col = "lightblue",
     border = "black")
# The way we learnt it using ggplot:
#ggplot(random_sample, aes(x = dep_delay)) +
...
ggplot(delay_data) + geom_freqpoly(aes(x = DepartureDelay),color = "red", binwidth = 10, alpha = 0.7) +
  geom_freqpoly(aes(x = ArrivalDelay),binwidth = 10, alpha = 0.7)  +
  labs(title = "Overlaid Frequency Polygons of Departure and Arrival Delays",
       x = "Delay (minutes)", y = "Frequency") 

# Alternative method of Create an overlaid frequency polygon using ggplot2
library(reshape2)
delay_data <- data.frame(DepartureDelay = random_sample$dep_delay,
                         ArrivalDelay = random_sample$arr_delay)
...
mode(random_sample$dep_delay)
# Function to calculate mode
calculate_mode <- function(x) {
  ux <- unique(x)
  ux[which.max(tabulate(match(x, ux)))]
}
#above display the first appearing number with highest freq
#if you need all modes with same higest freq:
Modes <- function(x) {
  ux <- unique(x)
...
mode_dep_delay <- calculate_mode(random_sample$dep_delay)
New_mode_dep_delay <- Modes(random_sample$dep_delay)
# Display the results
...
cat("Mode Departure Delay:", mode_dep_delay, "\n")
